...
PRIVATE aColorBackGrid:= {0,255,255} , aColorFontGrid := {0,0,0}
    ...
    DEFINE WINDOW Win_1  ;
      ...
      DEFINE TOOLBAR ToolBar_1 BUTTONSIZE 60,40 FLAT BORDER
          ...
          BUTTON Tools ;
             ...
             DEFINE DROPDOWN MENU BUTTON TOOLS
                    ITEM "Background color" Action MsgBox("This function will be created generating the program.","Save_color()") image "Raimbow"
     ...
     @ 430, 720 IMAGE Image_1 PICTURE 'MiniguiLogo.jpg' ACTION SelectControl('Image_1') WIDTH 200 HEIGHT 200 STRETCH TRANSPARENT ToolTip 'Demo Image'
     AddMnC("Image_1")
...
*------------------------------------------------------------------------------*
    ...
    WAIT WINDOW "Please wait while execute generation program..." NOWAIT
...
/
*-----------------------------------------------------------------------------*
Procedure Save_color()
*-----------------------------------------------------------------------------*
   local cor_atual, m_cor
   cor_atual := GetProperty ( "Win_regi", "BROWSE_1" , 'backcolor' )
   m_cor := GetColor( cor_atual )

   if m_cor[1] != NIL
      Begin ini file (c_pos+"Registro.Ini")
            Set Section "DEFAULT" ENTRY "RESPONSABILE"  To resp_def
            Set Section "DEFAULT" ENTRY "ANNO"  To annoesercizio
            Set Section "COLORI" ENTRY "Sfondo"  To m_cor
      End ini
      sfondo := m_cor
      SetProperty ( "Win_regi", "BROWSE_1" , 'backcolor' ,sfondo)
      SetProperty ( "Win_regi", "CheckBox_1" , 'backcolor' ,sfondo)
      SetProperty ( "Win_Regi", "BackColor" , Sfondo )
      DoMethod ( "Win_Regi", "Hide" )
      DoMethod ( "Win_Regi", "show" )
   Endif
return
/*
*/
