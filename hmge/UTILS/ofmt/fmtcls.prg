...
METHOD New( aParams, cIniName ) CLASS TFormatCode
   ...
   /* OOHG extensions */
   cOOHGCmds := "ACTIVATE,AS,BROWSE,BUTTON,BUTTONEX,CENTER,CHECKBOX,CHECKBUTTON,COL,COMBOBOX,FRAME,GRID,HYPERLINK,TEXTBOX,EDITBOX,LISTBOX,LOAD,PROGRESSBAR,RADIOGROUP,SPINNER,TAB,PAGE,POPUP,MENUITEM,ITEM,ITEMS,IMAGE,SEPARATOR,TOOLTIP,ALIGN,LEFT,RIGHT,ROW,OF,FONT,FONTBOLD,SIZE,NAME,ACTION,DROPDOWN,UPPERCASE,FLAT,BOLD,BACKCOLOR,FONTCOLOR,AUTOSIZE,BORDER,CENTERALIGN,RIGHTALIGN,SECTION,ENTRY,HEADERS,WIDTHS,OPTIONS,JUSTIFY,INPLACE,"
   cOOHGClss := "ACTION,AT,BALLOON,DEFINE,CAPTION,CLIENTAREA,CHANGE,ENTER,HEIGHT,ICON,INPUTMASK,INTERACTIVECLOSE,MAIN,MODAL,MAXLENGTH,MAXIMIZE,MULTIPLE,MENUSTYLE,MINIMIZE,NAVIGATION,NOMAXIMIZE,NOSIZE,NOTABSTOP,NUMERIC,OBJ,PROGID,READONLY,SMOOTH,STATUSBAR,STATUSITEM,TITLE,TOOLTIPSTYLE,VALUE,WARNING,WIDTH,WINDOW,FONTNAME,FONTSIZE,INCREMENT,RANGEMAX,RANGEMIN,ALIGNMENT,VCENTER,ONCHANGE,ONCLICK,ONDBLCLICK,ONENTER,MULTISELECT,ADDRESS,HANDCURSOR,SPACING,HORIZONTAL,TIMER,INTERVAL,ONCE,BACKCOLOR,FONTCOLOR,"
   cOOHGFuns := "GetProperty,SetProperty,DoMethod"
   ...
   ::cCommands := "," + ;
      "IF,ELSE,ELSEIF,END,ENDIF,DO,WHILE,ENDDO,WITH,OBJECT,ENDWITH,CASE,OTHERWISE,ENDCASE,BEGIN,ANNOUNCE,REQUEST,THREAD,DYNAMIC,EXTERNAL," + ;
   ...
   ::cClauses := "," + ;
      "ADDITIVE,ALIAS,ALL,BLANK,BOTTOM,BOX,CATCH,COLOR,DATE,DELETED,EACH,EXTENDED,EXCLUSIVE,FROM,GET,IN," + ;
      ...
      "PROTECTED,RELATION,SCOREBOARD,SEQUENCE,SOFTSEEK,STEP,STRUCTURE,TRY,TYPEAHEAD,UNIQUE,WRAP,TAG,TO,TOP,VALID,WHEN," + cOOHGClss
   IF ! Right( ::cClauses, 1 ) == ","
      ::cClauses += ","
   ENDIF
