...
PROCEDURE Main( ... )
   ...
   LOCAL oRef, aParams, cFileName, cInitDir, i, cParam, lRecursive := .F., bMainBlock
   ...
   DEFINE WINDOW frm_Main ;
      ...
      NOMAXIMIZE ;
      NOSIZE ;
      ON INIT ( frm_Main.Closable := .F., Eval( bMainBlock ) )
      ...
      ON KEY ESCAPE ACTION iif( frm_Main.Closable, frm_Main.Release(), NIL )
   ...
   IF "*" $ cFileName
      ...
      bMainBlock := { || DirEval( cInitDir, cFileName, lRecursive, {| name | Reformat( oRef, name ) } ), frm_Main.Closable := .T. }
   ELSE
      bMainBlock := { || Reformat( oRef, cFileName ), frm_Main.Closable := .T. }
   ENDIF
...
STATIC PROCEDURE Reformat( oRef, cFileName )
   ...
   IF Empty( aFile := oRef:File2Array( cFileName ) )
      cStat := frm_Main.edt_Status.Value
      cStat += "File " + cFileName + " not found !" + Chr( 13 ) + Chr( 10 )
      frm_Main.edt_Status.Value := cStat
   ELSE
      oRef:bCallBack := {| nItem, nTotal | ShowProgress( nItem, nTotal ) }

      cStat := frm_Main.edt_Status.Value
      cStat += "File " + cFileName
      frm_Main.edt_Status.Value := cStat

      IF oRef:Reformat( aFile )
         oRef:Array2File( cFileName, aFile )
         cStat := frm_Main.edt_Status.Value
         cStat += ", " + hb_ntos( Len( aFile ) ) + " lines reformatted." + Chr( 13 ) + Chr( 10 )
         frm_Main.edt_Status.Value := cStat
      ELSE
         cStat := frm_Main.edt_Status.Value
         cStat += ", error " + hb_ntos( oRef:nErr ) + " on line " + hb_ntos( oRef:nLineErr ) + ": " + oRef:cLineErr + Chr( 13 ) + Chr( 10 )
         frm_Main.edt_Status.Value := cStat
      ENDIF
   ENDIF
...
STATIC PROCEDURE About()
   ...
   MsgInfo( ;
      "Source Code Formatter" + Chr( 13 ) + Chr( 10 ) + ;
