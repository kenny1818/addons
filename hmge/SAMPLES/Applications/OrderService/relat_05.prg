...
FUNCTION OS_cliente_1()
   ...
   DEFINE WINDOW form_os_cliente1 ;
         AT 000, 000 ;
         WIDTH 315 ;
         HEIGHT 220 ;
         TITLE 'Relatório: OS em Aberto (por cliente e período)' ;
         ICON 'icone' ;
         modal ;
         NOSIZE
      ...
      DEFINE BUTTONEX btn_volta
         ...
         FONTBOLD .T.
         lefttext .F.
         ACTION form_os_cliente1.RELEASE
      END BUTTONEX

   END WINDOW

   form_os_cliente1.CENTER
   form_os_cliente1.ACTIVATE

return( nil )
// -------------------------------------------------------------------------------
FUNCTION Imprime_OS_cliente_1()
   ...
   PRIVATE v_cliente := AllTrim( form_os_cliente1.cbo_cliente.item( form_os_cliente1.cbo_cliente.value ) )

   dData_01 := td( form_os_cliente1.dpi_de.value )
   dData_02 := td( form_os_cliente1.dpi_ate.value )

   oQuery := oServer:Query( "select * from os where data>='" + dData_01 + "' and data<='" + dData_02 + "' and nome_cliente='" + v_cliente + "' and encerrado=0 order by data" )
   ...
   IF lSuccess == .T.
      ...
      cabecalho_os_periodo( pagina, form_os_cliente1.dpi_de.VALUE, form_os_cliente1.dpi_ate.value )
   ...
   */
   ...
   form_os_cliente1.RELEASE

return( nil )
// -------------------------------------------------------------------------------
STATIC FUNCTION cabecalho_os_periodo( pPagina, dData1, dData2 )

   @ 005, 010 PRINT 'RELAÇÃO DE ORDENS DE SERVIÇO ABERTAS - (Por CLIENTE e por Período)' FONT 'courier new' SIZE 10 BOLD
