...
STATIC FUNCTION dados( parametro )
   ...
   LOCAL x_id := AllTrim( valor_coluna( 'grid_clientes', 'form_clientes', 1 ) )
   ...
   IF parametro == 2
      IF Empty( x_id )
         ...
         return( nil )
      ELSE
         oQuery := oServer:Query( 'select * from clientes where id = ' + x_id )
         IF oQuery:NetErr()
      ...
      x_fixo := AllTrim( oRow:fieldGet( 6 ) )
      x_celular := AllTrim( oRow:fieldGet( 7 ) )
      x_endereco := AllTrim( oRow:fieldGet( 8 ) )
      x_numero := AllTrim( oRow:fieldGet( 9 ) )
      x_complem := AllTrim( oRow:fieldGet( 10 ) )
      x_bairro := AllTrim( oRow:fieldGet( 11 ) )
      x_cidade := AllTrim( oRow:fieldGet( 12 ) )
      x_uf := AllTrim( oRow:fieldGet( 13 ) )
      x_cep := AllTrim( oRow:fieldGet( 14 ) )
      x_email := AllTrim( oRow:fieldGet( 15 ) )
      x_aniv_dia := oRow:fieldGet( 16 )
      x_aniv_mes := oRow:fieldGet( 17 )
      x_cnpj := AllTrim( oRow:fieldGet( 2 ) )
      x_cpf := AllTrim( oRow:fieldGet( 3 ) )
      x_i_estadual := AllTrim( oRow:fieldGet( 4 ) )
      oQuery:Destroy()
   ENDIF
...
STATIC FUNCTION excluir()
   ...
   LOCAL v_id := AllTrim( valor_coluna( 'grid_clientes', 'form_clientes', 1 ) )
   LOCAL v_nome := AllTrim( valor_coluna( 'grid_clientes', 'form_clientes', 2 ) )
   ...
   IF msgyesno( 'Confirma a exclusão de : ' + v_nome + ' ?' )
      cQuery := 'delete from clientes where id = ' + v_id
...
STATIC FUNCTION executa_relatorio_001()
   ...
   FOR n_i := 1 TO oQuery:LastRec()
      ...
      @ linha, 045 PRINT AllTrim( oRow:fieldGet( 5 ) ) FONT 'courier new' SIZE 10
      @ linha, 150 PRINT oRow:fieldGet( 6 ) + '-' + oRow:fieldGet( 7 ) FONT 'courier new' SIZE 10
...
STATIC FUNCTION cabecalho_1( p_pagina )
   ...
   @ 010, 050 PRINT 'RELAÇÃO DE CLIENTES' FONT 'courier new' SIZE 018 BOLD
...
STATIC FUNCTION gravar( parametro )
   ...
   LOCAL v_aniv_dia := AllTrim( Str( form_dados.tbox_012.value ) )
   LOCAL v_aniv_mes := AllTrim( Str( form_dados.tbox_013.value ) )
   LOCAL v_cnpj := AllTrim( form_dados.tbox_014.value )
   LOCAL v_cpf := AllTrim( form_dados.tbox_015.value )
   LOCAL v_i_estadual := AllTrim( form_dados.tbox_016.value )
   ...
   IF parametro == 1
      IF Empty( v_nome )
         ...
         return( nil )
      ELSE
         cQuery := "insert into clientes (nome,fixo,celular,endereco,numero,complemento,bairro,cidade,uf,cep,email,aniv_dia,aniv_mes,cnpj,cpf,insc_est,data_cad,hora_cad) values ('"
         cQuery += v_nome + "','"
         cQuery += v_fixo + "','"
         cQuery += v_celular + "','"
         cQuery += v_endereco + "','"
   ...
   ELSEIF parametro == 2
      ...
      ELSE
         ...
         cQuery += "aniv_dia='" + v_aniv_dia + "',"
         cQuery += "aniv_mes='" + v_aniv_mes + "',"
         cQuery += "cnpj='" + v_cnpj + "',"
         cQuery += "cpf='" + v_cpf + "',"
         cQuery += "insc_est='" + v_i_estadual + "'"
...
STATIC FUNCTION pesquisar()
   ...
   LOCAL v_conteudo := form_clientes.tbox_pesquisa.VALUE
   LOCAL v_pesquisa := '"' + Upper( AllTrim( form_clientes.tbox_pesquisa.value ) ) + '%"'
   LOCAL n_i := 0
   LOCAL oRow := {}
   ...
   IF Empty( v_conteudo )
      oQuery := oServer:Query( 'select id,nome,fixo,celular from clientes order by nome' )
   ELSE
      oQuery := oServer:Query( 'select id,nome,fixo,celular from clientes where nome like ' + v_pesquisa + ' order by nome' )
   ENDIF
   ...
   IF oQuery:Eof()
      // msginfo('Sua pesquisa não foi encontrada','Atenção')
      return( nil )
   ENDIF

   FOR n_i := 1 TO oQuery:LastRec()
      oRow := oQuery:GetRow( n_i )
      add ITEM { Str( oRow:fieldGet( 1 ) ), AllTrim( oRow:fieldGet( 2 ) ), AllTrim( oRow:fieldGet( 3 ) ), AllTrim( oRow:fieldGet( 4 ) ) } TO grid_clientes OF form_clientes
      oQuery:Skip( 1 )
   NEXT n_i
