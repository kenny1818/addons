...
// -------------------------------------------------------------------------------
FUNCTION OS_af_periodo()

   DEFINE WINDOW form_os_periodo ;
         AT 000, 000 ;
         WIDTH 315 ;
         HEIGHT 160 ;
         TITLE 'Relatório: OS em Aberto (por período)' ;
         ICON 'icone' ;
         modal ;
         NOSIZE
      ...
      DEFINE BUTTONEX btn_volta
         ...
         FONTBOLD .T.
         lefttext .F.
         ACTION form_os_periodo.RELEASE
      END BUTTONEX

   END WINDOW

   form_os_periodo.CENTER
   form_os_periodo.ACTIVATE

return( nil )
// -------------------------------------------------------------------------------
FUNCTION Imprime_OS_Periodo()
   ...
   dData_01 := td( form_os_periodo.dpi_de.value )
   dData_02 := td( form_os_periodo.dpi_ate.value )

   oQuery := oServer:Query( "select * from os where data>='" + dData_01 + "' and data<='" + dData_02 + "' and encerrado=0 order by data" )
   ...
   IF lSuccess == .T.
      ...
      cabecalho_os_periodo( pagina, form_os_periodo.dpi_de.VALUE, form_os_periodo.dpi_ate.value )
   ...
   form_os_periodo.RELEASE

return( nil )
// -------------------------------------------------------------------------------
FUNCTION cabecalho_os_periodo( pPagina, dData1, dData2 )

   @ 005, 010 PRINT 'RELAÇÃO DE ORDENS DE SERVIÇO ABERTAS - (Por Período)' FONT 'courier new' SIZE 10 BOLD
