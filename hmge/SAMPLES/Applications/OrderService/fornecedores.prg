...
STATIC FUNCTION dados( parametro )
   ...
   LOCAL x_id := AllTrim( valor_coluna( 'grid_fornecedores', 'form_fornecedores', 1 ) )
   ...
   IF parametro == 2
      IF Empty( x_id )
         ...
         return( nil )
      ELSE
         oQuery := oServer:Query( 'select * from fornecedores where id = ' + x_id )
         IF oQuery:NetErr()
...
STATIC FUNCTION excluir()
   ...
   LOCAL v_id := AllTrim( valor_coluna( 'grid_fornecedores', 'form_fornecedores', 1 ) )
   LOCAL v_nome := AllTrim( valor_coluna( 'grid_fornecedores', 'form_fornecedores', 2 ) )
   ...
   IF msgyesno( 'Confirma a exclusão de : ' + v_nome + ' ?' )
      cQuery := 'delete from fornecedores where id = ' + v_id
...
STATIC FUNCTION cabecalho_1( p_pagina )
   ...
   @ 010, 050 PRINT 'RELAÇÃO DE FORNECEDORES' FONT 'courier new' SIZE 018 BOLD
...
STATIC FUNCTION gravar( parametro )
   ...
   IF parametro == 1
      IF Empty( v_nome )
         ...
         return( nil )
      ELSE
         cQuery := "insert into fornecedores (nome,fixo,celular,endereco,numero,complemento,bairro,cidade,uf,cep,email,cnpj,cpf,insc_est,data_cad,hora_cad) values ('"
         cQuery += v_nome + "','"
         cQuery += v_fixo + "','"
         cQuery += v_celular + "','"
         cQuery += v_endereco + "','"
   ...
   ELSEIF parametro == 2
      IF Empty( v_nome )
         ...
         return( nil )
      ELSE
         cQuery := "update fornecedores set "
         cQuery += "nome='" + v_nome + "',"
         cQuery += "fixo='" + v_fixo + "',"
...
STATIC FUNCTION pesquisar()
   ...
   LOCAL v_conteudo := form_fornecedores.tbox_pesquisa.VALUE
   LOCAL v_pesquisa := '"' + Upper( AllTrim( form_fornecedores.tbox_pesquisa.value ) ) + '%"'
   LOCAL n_i := 0
   LOCAL oRow := {}
   ...
   IF Empty( v_conteudo )
      oQuery := oServer:Query( 'select id,nome,fixo,celular from fornecedores order by nome' )
   ELSE
      oQuery := oServer:Query( 'select id,nome,fixo,celular from fornecedores where nome like ' + v_pesquisa + ' order by nome' )
   ENDIF
   ...
   FOR n_i := 1 TO oQuery:LastRec()
      oRow := oQuery:GetRow( n_i )
      add ITEM { Str( oRow:fieldGet( 1 ) ), AllTrim( oRow:fieldGet( 2 ) ), AllTrim( oRow:fieldGet( 3 ) ), AllTrim( oRow:fieldGet( 4 ) ) } TO grid_fornecedores OF form_fornecedores
      oQuery:Skip( 1 )
   NEXT n_i
