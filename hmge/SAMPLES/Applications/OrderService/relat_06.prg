/*
  propósito |relatórios de ordens de serviço - abertas - por período
  parâmetros|nenhum
  retorno   |nil
*/
...
FUNCTION OS_cliente_2()
   ...
   FOR n_i := 1 TO oQuery:LastRec()
      ...
      AAdd( a_cliente, oRow:fieldGet( 5 ) )
      oQuery:Skip( 1 )
   NEXT n_i

   DEFINE WINDOW form_os_cliente2 ;
         AT 000, 000 ;
         WIDTH 315 ;
         HEIGHT 220 ;
         TITLE 'Relatório: OS Encerrada (por cliente e período)' ;
         ICON 'icone' ;
         modal ;
         NOSIZE
      ...
      @ 060, 005 LABEL lbl_cliente ;
         WIDTH 160 ;
         VALUE 'Escolha o Cliente' ;
         FONTCOLOR BLUE BOLD
      @ 080, 005 comboboxex cbo_cliente WIDTH 300 ITEMS a_cliente HEIGHT 400 listwidth 300 VALUE 1 FONT 'verdana' SIZE 10
      ...
      DEFINE BUTTONEX btn_volta
         ...
         FONTBOLD .T.
         lefttext .F.
         ACTION form_os_cliente2.RELEASE
      END BUTTONEX

   END WINDOW

   form_os_cliente2.CENTER
   form_os_cliente2.ACTIVATE

return( nil )
// -------------------------------------------------------------------------------
FUNCTION Imprime_OS_cliente_2()
   ...
   PRIVATE v_cliente := AllTrim( form_os_cliente2.cbo_cliente.item( form_os_cliente2.cbo_cliente.value ) )

   dData_01 := td( form_os_cliente2.dpi_de.value )
   dData_02 := td( form_os_cliente2.dpi_ate.value )

   oQuery := oServer:Query( "select * from os where data>='" + dData_01 + "' and data<='" + dData_02 + "' and nome_cliente='" + v_cliente + "' and encerrado=1 order by data" )
   ...
   IF lSuccess == .T.
      ...
      cabecalho_os_periodo( pagina, form_os_cliente2.dpi_de.VALUE, form_os_cliente2.dpi_ate.value )
   ...
   */
   ...
   form_os_cliente2.RELEASE

return( nil )
// -------------------------------------------------------------------------------
STATIC FUNCTION cabecalho_os_periodo( pPagina, dData1, dData2 )

   @ 005, 010 PRINT 'RELAÇÃO DE ORDENS DE SERVIÇO ENCERRADAS - (Por CLIENTE e por Período)' FONT 'courier new' SIZE 10 BOLD
   @ 010, 010 PRINT 'CLIENTE : ' + v_cliente FONT 'courier new' SIZE 10 BOLD
   @ 015, 010 PRINT 'PERÍODO : ' + DToC( dData1 ) + ' até ' + DToC( dData2 ) FONT 'courier new' SIZE 10
   @ 020, 010 PRINT 'EMISSÃO : ' + Chk_DiaSem( Date(), 2 ) + ', ' + AllTrim( Str( Day( Date() ) ) ) + ' de ' + Chk_Mes( Month( Date() ), 1 ) + ' de ' + StrZero( Year( Date() ), 4 ) + ' - ' + Time() + 'h.' FONT 'courier new' SIZE 10
   @ 025, 010 PRINT 'PÁGINA  : ' + StrZero( pPagina, 3 ) FONT 'courier new' SIZE 10
