...
FUNCTION OS_tecnico_2()
   ...
   DEFINE WINDOW form_os_tecnico2 ;
         AT 000, 000 ;
         WIDTH 315 ;
         HEIGHT 220 ;
         TITLE 'Relatório: OS Encerrada (por técnico e período)' ;
         ICON 'icone' ;
         modal ;
         NOSIZE
      ...
      DEFINE BUTTONEX btn_volta
         ...
         FONTBOLD .T.
         lefttext .F.
         ACTION form_os_tecnico2.RELEASE
      END BUTTONEX

   END WINDOW

   form_os_tecnico2.CENTER
   form_os_tecnico2.ACTIVATE

return( nil )
// -------------------------------------------------------------------------------
FUNCTION Imprime_OS_tecnico_2()
   ...
   PRIVATE v_tecnico := AllTrim( form_os_tecnico2.cbo_tecnico.item( form_os_tecnico2.cbo_tecnico.value ) )

   dData_01 := td( form_os_tecnico2.dpi_de.value )
   dData_02 := td( form_os_tecnico2.dpi_ate.value )

   oQuery := oServer:Query( "select * from os where data>='" + dData_01 + "' and data<='" + dData_02 + "' and nome_atendente='" + v_tecnico + "' and encerrado=1 order by data" )
   ...
   IF lSuccess == .T.
      ...
      cabecalho_os_periodo( pagina, form_os_tecnico2.dpi_de.VALUE, form_os_tecnico2.dpi_ate.value )
   ...
   */
   ...
   form_os_tecnico2.RELEASE

return( nil )
// -------------------------------------------------------------------------------
STATIC FUNCTION cabecalho_os_periodo( pPagina, dData1, dData2 )

   @ 005, 010 PRINT 'RELAÇÃO DE ORDENS DE SERVIÇO ENCERRADAS - (Por TÉCNICO e por Período)' FONT 'courier new' SIZE 10 BOLD
   @ 010, 010 PRINT 'TÉCNICO : ' + v_tecnico FONT 'courier new' SIZE 10 BOLD
