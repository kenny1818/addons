...
*--------------------------------------------------------*
Static Function GETREGVAR(nKey, cRegKey, cSubKey)
*--------------------------------------------------------*
   LOCAL cValue

   nKey := IF(nKey == NIL, HKEY_CURRENT_USER, nKey)
   cValue := win_regGet( nKey, cRegKey, cSubKey )
...
*--------------------------------------------------------*
   ...
   lValue := win_regSet( nKey, cRegKey, cSubKey, uValue ) 
...
*--------------------------------------------------------*
   ...
   nKey := IF(nKey == NIL, HKEY_CURRENT_USER, nKey)
   IF win_regOpenKeyEx( nKey, cRegKey, 0, KEY_SET_VALUE, @pKeyHandle )
      lValue := win_regDeleteValue( pKeyHandle, cSubKey )
...
HB_FUNC ( SOCKET_SETTIME )
{
   ...
   HB_LONGLONG Time_Int64;

   Time_Int64 = Int32x32To64( lnTime, 10000000 ) + 116444736000000000LL;
   FileTime.dwLowDateTime = ( DWORD ) Time_Int64;
   FileTime.dwHighDateTime = ( DWORD ) ( Time_Int64 >> 32 );
