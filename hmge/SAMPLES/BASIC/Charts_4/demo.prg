...
Local rs := CreateObject("ADODB.Recordset")
   ...
   TRY
      cn:Open("DSN=SQLite3 Datasource;Database=demo.sq3;" + ;
              "StepAPI=0;SyncPragma=NORMAL;NoTXN=0;Timeout=1000;ShortNames=0;LongNames=0;NoCreat=0;NoWCHAR=0;FKSupport=0;JournalMode=;OEMCP=0;LoadExt=;BigInt=0;JDConv=0;PWD=")
   CATCH
      MsgStop( 'This Program Required Installed SQLite ODBC Driver!', 'Stop' )
      QUIT
   END
   ...
   //
   sql := "SELECT * FROM Country ORDER BY Area DESC LIMIT 10"
   ...
   //
   sql := "SELECT Name, Population/Area as off FROM Country ORDER BY Population/Area DESC LIMIT 20"
