 ...
 * (c) Roberto López [harbourminigui@gmail.com]
...
procedure AdvancedDemo( cArea )
        ...
        LOCAL aFieldEdit   := { .t., .t., .t., .t., .t., .t., .t., .t., .f., .t., .t. }
        LOCAL aOptions     := Array( 3, 2 )
        LOCAL bSave        := {|aValues, lNew| AdvancedSave( aValues, lNew, cArea ) }
        LOCAL bSearch      := {|| MsgInfo( "Your own search function", ThisWindow.Title ) }
        LOCAL bPrint       := {|| MsgInfo( "Your own print function", ThisWindow.Title ) }
        aOptions[1,1] := "Execute option 1"
        aOptions[1,2] := {|| MsgInfo( "You can do something here 1", ThisWindow.Title ) }
        aOptions[2,1] := "Execute option 2"
        aOptions[2,2] := { || MsgInfo( "You can do something here 2", ThisWindow.Title ) }
        aOptions[3,1] := "Execute option 3"
        aOptions[3,2] := { || MsgInfo( "You can do something here 3", ThisWindow.Title ) }
...
procedure SelectLang()
        ...
        nItem := SelectItem( aLangName, aLangID )
        IF nItem != 0
...
function SelectItem( acItems, acLangs )
        ...
        // Activate the window.------------------------------------------------
        wndSelItem.lbxItems.Value := Ascan( acLangs, {|lang| Lower( Left( lang, 2 ) ) == Left( Set( _SET_LANGUAGE ), 2 )} )
...
procedure About()
        ...
        cMessage += "---> Dutch - Ready (Thanks to René Koot)    " + CRLF
        cMessage += "---> Polish - Ready (Thanks to Jacek Kubica)    " + CRLF
        cMessage += "---> German - Ready (Thanks to Andreas Wiltfang)   " + CRLF
        cMessage += CRLF
        cMessage += "Please report bugs to HMG discussion group at groups.yahoo.com     " + CRLF
