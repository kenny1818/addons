...
// With this method only this prg file needs to be included to your own project.
// ****************************************************************************************************************

// Version info
#define  Version        "V1.22"
#define  Versiondate    "09/05/2021"
...
//
// ( add others type here, the rest of the program will follow )
//
...
MEMVAR th_w_width // OTIS main window width
MEMVAR th_w_height // OTIS main window height
MEMVAR th_w_ctrlgap // OTIS main window border and control gap
MEMVAR th_bt_width // menu button width
MEMVAR th_bt_height // menu button height

MEMVAR th_w_fontcolor // window font color
MEMVAR th_w_bgcolor // window bg color

MEMVAR th_bt_fontcol // font color
MEMVAR th_bt_bgcol // font bg color
MEMVAR th_bt_ohfontcol // on hover font color
MEMVAR th_bt_ohbgcol // on hover bg color
         ...
         FOR i := 1 TO Len( aTokens )
            ...
            CATCH oError
               ...
               IF At( "DOS Error 32", temp ) <> 0
                  temp += crlf + crlf + 'File is probably in readonly mode.'
               ENDIF
               MsgStop( temp )
            END
...
STATIC FUNCTION otis_release()
   ...
   // it permits us to open and close without other mouse clicks.
   // When you are in command line mode and add a file than the flag will be set
   // with result that on exit Otis asks to save the dataset on exit.
   IF lDsChanged
...
STATIC FUNCTION otis_add_index( oBrowse )
   ...
   LOCAL cOrdBagext, nOldsel, aSelected := {}, cTablename, i
...
STATIC FUNCTION table_export( cTablename, cAlias, aColVis, cType )
   ...
   ELSEIF cType == 'clipboard'
      ...
      cClipboard := "// " + crlf + ;
         ...
         "// " + crlf + ;
         "// Structure of table : " + cTablename + crlf + ;
         "// " + crlf + ;
         "// " + "Nr" + repl( Chr( 9 ), 3 ) + ;
...
STATIC FUNCTION index_mng_new2( cWinname, cFilename, cTag, cKey, cFor, lUnique, nAscDes )
   ...
   IF lOk
      ...
      IF lOk
         ...
         CATCH oError
            MsgStop( "Order create error probably because" + crlf + ;
...
STATIC FUNCTION dv_scope( cWinname, cAction )
   ...
   LOCAL bApply := {||
   // if dv_scope2(cTopExpr, cBottomExpr)
   // set scope
   ( cAlias )->( ordScope( TOPSCOPE, cTopExpr ) )
   ( cAlias )->( ordScope( BOTTOMSCOPE, cBottomExpr ) )
...
STATIC FUNCTION dv_ai_rec( cWinname )
   ...
   DEFINE WINDOW &f_ai ;
      ...
      IF ( cAlias )->( dbInfo( DBI_SHARED ) )
         SetProperty( ThisWindow.NAME, "bt_insert", "Enabled", .F. )
...
STATIC FUNCTION dv_pupd_rec( cWinname, cAction )
   ...
   ELSE
      msginfo( "Push down / Pull up a record is only" + crlf + ;
         "possible if there is no order active." )
   ENDIF
...
STATIC FUNCTION dv_pack_zap( cWinname, cAction )
   ...
   LOCAL lTemp, nReccount, nSecStart
   ...
   ELSE
      msgstop( "Impossible, the table was not open in EXCLUSIVE mode." )
   ENDIF
...
// ------------------------------------------------------------------------------*
STATIC PROCEDURE dv_Search_Replace( lReplace, cWinname )
   // ------------------------------------------------------------------------------*
...
// ***********************************************************************************************************************************
//
// Some parts of this code below is borrowed from "MGDBU" from "Grigory Filatov".
...
STATIC FUNCTION Struct_Editor()
   ...
   // code block Ok button
   LOCAL bOK := {||
   _HMG_DialogCancelled := ! BRW_2:lHasChanged
   aStruct := BRW_2:aArray
   DoMethod( 'frmTableNew', 'Release' )

RETURN NIL
}

// code block Cancel button
LOCAL bCancel := {||
IF BRW_2:lHasChanged
PlayExclamation()
HMG_ModalDialogReturn := 2
...
// get filename
cFileDbf := PutFile( { { "File DBF (*.DBF)", "*.DBF" }, { 'All Files', '*.*' } }, 'New table...', GetCurrentFolder() )

// return if get filename canceled
IF Empty( cFileDbf )
RETURN NIL
ENDIF

// rename a possible old file but ask confirmation.
IF File( cFileDbf )
PlayExclamation()
IF MsgOkCancel( "This file exists already and will be renamed to <" + cFileNoExt( cFileDbf ) + ".bak>." )
FRename( cFileDbf, cFileNoExt( cFileDbf ) + ".bak" )
ENDIF
ENDIF

// fieldtype full word to single char conversion
...
// release struct editor form
STATIC FUNCTION se_release()

   // release form modify field name, type, len, ...
   IF ISWINDOWDEFINED( "FieldNew" )
      DoMethod( "FieldNew", "RELEASE" )
   ENDIF

   // release field type info form
   IF ISWINDOWDEFINED( "f_extfldinfo" )
      DoMethod( "f_extfldinfo", "RELEASE" )
   ENDIF
...
STATIC FUNCTION se_resize()
   ...
   SetProperty( ThisWindow.NAME, "btnExit", "row", GetProperty( ThisWindow.NAME, "ClientHeight" ) - ( th_bt_height + th_w_ctrlgap ) * 2 )
   SetProperty( ThisWindow.NAME, "btnOK", "row", GetProperty( ThisWindow.NAME, "ClientHeight" ) - ( th_bt_height + th_w_ctrlgap ) * 1 )
            ...
            ELSE
               ...
               ELSE
                  ...
                  IF Len( charonly( "_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ", cField ) ) <> Len( cField )
                     MsgStop( "Only the following characters, 0..9_A..Z, are allowed in a fieldname." )
                     DoMethod( "FieldNew", "edtFld", "SetFocus" )
...
STATIC FUNCTION se_load_struct( cFormName )
   ...
   LOCAL temp, i, x, cFn, aStruct, nOldsel, cRdd
   LOCAL oBrw := GetBrowseObj( "BRW_2", cFormName )
   ...
   if ! Empty( cFn )
      ...
      TRY
         // use (cFn) ALIAS IMPSTRUCT EXCLUSIVE //SHARED
      ...
      CATCH oError
         MsgStop( "OTIS can not load the structure." + crlf + ;
            "The file is probably opened by this or" + crlf + ;
            "another program in exclusive mode." + crlf + ;
            "or the wrong Rdd is used" + crlf + ;
            "or a unknown field type is found." ;
            )
         RETURN NIL
      END
...
//
// ***************************************************************
STATIC FUNCTION Leto_getfiles( cPath, cMask )
// ***************************************************************
...
STATIC FUNCTION show_info( cTablename, cInfo )
   ...
   DEFINE WINDOW &form_tii ;
         AT 200, 300 ;
         CLIENTAREA 450, Min( th_w_ctrlgap * 2 + th_bt_height + MLCount( cInfo ) * 14 + 70, GetDesktopHeight() - 350 ) ; // 14 = fontsize + spacing (defined by test), 50 window borders
      TITLE cTablename ;
         BACKCOLOR th_w_bgcolor ;
         WINDOWTYPE STANDARD ;
...
// Show help window
STATIC FUNCTION show_help( cInfo )
   ...
   FOR i := 1 TO nLinecnt
      ...
      IF ( temp := Len( RTrim( cLine ) ) ) > nMaxLen
         nMaxLen := temp
      ENDIF
   NEXT i
   // msgstop(nMaxLen)
   ...
   */
   ...
   DEFINE WINDOW f_help ;
         AT 0, 0 ;
         CLIENTAREA nMaxlen * 05.6, nLinecnt * 19 ;
         TITLE "OTIS - Info" ;
         NOSIZE ;
         ;// NOMINIMIZE ;
      ;// NOMAXIMIZE ;
   ...
   // CENTER WINDOW
   SetProperty( "f_help", "Row", GetDesktopHeight() - GetProperty( "f_help", "Height" ) - GetTaskBarHeight() - 15 )
   SetProperty( "f_help", "Col", GetDesktopWidth() - GetProperty( "f_help", "Width" ) - 15 )
...
STATIC FUNCTION set_theme( cThemeId )
   ...
   CASE cThemeId == "2"
      ...
      th_bt_height := 28 // menu button height
