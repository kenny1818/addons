...
Function TBrw_Create( ControlName, ParentForm, nRow, nCol, nWidth, nHeight, uAlias, lCell, FontName, FontSize, Backcolor )
   ...
   Local uSelector := NIL, lAutoCol := .F., aColSel := NIL, lEditable := .F.
    ...
    If nWidth  < 0                                         // decrease width
       nWdt   := nWidth
       nWidth := 0
    EndIf
    
    If nWidth == 0                                         // calculate the width
       nWidth := aRect[3]
    EndIf
    
    If nHeight < 0                                         // decrease height
       nHgt    := nHeight
       nHeight := 0
    EndIf
    
    If nHeight == 0                                        // calculate height
       nHeight := aRect[4] - nRow
    EndIf
    
    If nHgt < 0; nHeight += nHgt                           // reduce height
    EndIf
    
    If nWdt < 0; nWidth  += nWdt                           // reduce the width
    EndIf
...
Function TBrw_NoHoles( oBrw, nDelta, lSet ) 
   ...
   If lSet
       ...
       If nI > 0                          // there are headers
      
          nK := int( nHole / nI )         // by nI - headers divide the hole
       ...
       Else             // no headers, you can reduce the size of tsb by size nHole
