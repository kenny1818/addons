...
//   ALTD()
   ...
   ? "nakl1", Used(), Alias(), OrdCount(), IndexOrd(), OrdSetFocus(), IndexKey()
   use ( cPath+"nakl2" ) Shared New
   index on Dtos(DORD)+Str(NORD,10,0)+Str(NPROD,3,0) TAG DATA
   ? "nakl2", Used(), Alias(), OrdCount(), IndexOrd(), OrdSetFocus(), IndexKey()
   ...
   FOR nTrans := 1 TO 250
      ...
      IF ! ChkNakl( nRec, nTrans, Date(), { 1400.5, 28632.28, 800.51 } )
         ? "internal Transaction problem ", nTrans
         lRet := .F.
      ENDIF
      ...
      if dbSeek( Dtos(Date())+Str(nTrans,10)+Str(2,3) )
         if ChangeNakl( 35688.24 )
            ...
            ? "Failure - Rollback", nTrans, 'ChangeNakl( 35688.24 )'
            lRet := .F.
         endif
         ...
         if NoChangeNakl( 35688.24 )
            ...
            ? "Failure in Rollback", nTrans, 'NoChangeNakl( 35688.24 )'
            lRet := .F.
         endif
      endif
      IF ! lRet
   ...
   ? "Done", STR( ( hb_milliseconds() - nSec ) / 1000, 7, 2 ), "s", nTrans
   ?
