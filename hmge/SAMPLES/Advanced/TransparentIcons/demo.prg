   ...
   aadd(M->aColors,RGB(235, 245, 214)) ; aadd(aThemesName, "LightGreen"        ) ; aadd( aImages , 'BMP00' ) 
     ...
     DEFINE COMBOBOXEX Combo_1 
       		...
       		IMAGE	aImages
                ...
                ONCHANGE ChangeThemes(This.Value)
     END COMBOBOXEX
     ...
     END BUTTONEX  	 
      ...
      DEFINE STATUSBAR
         STATUSITEM COPYRIGHT BACKCOLOR RGB2DIM( aColors[nColor] )
         DATE BACKCOLOR RGB2DIM( aColors[nColor] )
         CLOCK BACKCOLOR RGB2DIM( aColors[nColor] )
      END STATUSBAR
...
Function ChangeThemes(nValue)
   ...
   IF ( h := MyGetControlHandle( 'StatusBar' ) ) > 0
      SetSbBkColor( h, aSetColor )
      InvalidateRect( GetFormHandle( MyForm ), 0 )
   ENDIF
