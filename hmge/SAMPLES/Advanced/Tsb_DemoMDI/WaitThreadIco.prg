 ...
 * «агружены handle иконок в массив дл€ использовани€ внутри дл€ DRAW ICON ...
 * символьное им€ иконки из ресурсов hmg
*/
...
FUNCTION WaitThreadCreateIcon( cTitle, cIndicator )
   ...
   hStatPictWait := array(len(aStatPictWait))
   FOR nRow := 1 TO len(aStatPictWait)
       hStatPictWait[nRow] := LoadIconByName( aStatPictWait[nRow], 64, 64 )
   NEXT
   ...
   DEFINE WINDOW &cFormName     ;
     ...
     DRAW ICON IN WINDOW &cFormName AT 20, 20 PICTURE hStatPictWait[1] WIDTH 64 HEIGHT 64 TRANSPARENT
...
FUNCTION WaitThreadCloseIcon()
   LOCAL cFormName := aStaticParam[1], nRow
   ...
   //IF _IsWindowActive( cFormName )
      ...
      DO MESSAGE LOOP
   //ENDIF
   FOR nRow := 1 TO len(hStatPictWait)
       DeleteObject( hStatPictWait[nRow] )
   NEXT
   hStatPictWait := NIL
