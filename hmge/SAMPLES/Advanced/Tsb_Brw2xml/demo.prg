...
PROCEDURE Main( Line )
   ...
   DEFINE FONT Font_8  FONTNAME cFontName8 SIZE nFontSize 
   DEFINE FONT Font_9  FONTNAME cFontName1 SIZE 9             
...
STATIC FUNCTION TsbColor( oBrw, aHColor, aBColor, nHClr1, nHClr2 )
   LOCAL Khaki := CLR_KHAKI     // RGB(255,255,74)
   LOCAL Yellow := CLR_YELLOW  // RGB(240,240,0)
   DEFAULT aHColor := {255, 255, 255} , ;  // CLR_WHITE    Цвет шапки таблицы
           aBColor := {169, 169, 169} , ;  // CLR_DARKGRAY Цвет фона  таблицы
           nHClr1  := CLR_WHITE       , ;  // MyRGB( aHColor ), ;
           nHClr2  := CLR_DARKSLATEGRAY    // MyRGB( { 51, 51, 51 } )
   ...
   WITH OBJECT oBrw  // oBrw объект установлен\зарегистрирован
     ...
     :SetColor( {12}, { { || { Khaki          , Yellow        }} } ) // 12, фона неактивного курсора (selected cell no focused)   
...
STATIC FUNCTION TsbColorBack( nAt, nCol, oBrw )
   ...
   ELSEIF nTsbColor == 6    // the color of the table "multicolor"
      IF     nClr3 == 1
         nColor := CLR_SKYBLUE // RGB(159,191,236)  // Office_2003 Blue
      ELSEIF nClr3 == 2
         nColor := CLR_SEASHELL // RGB(234,240,207)  // Office_2003 GREEN 
      ELSEIF nClr3 == 3
         nColor := CLR_PLUM // RGB(225,226,236)  // Office_2003 SILVER 
      ELSEIF nClr3 == 4
         nColor := CLR_MOCCASIN // RGB(251,230,148)  // Office_2003 ORANGE 
      ELSEIF nClr3 == 5
         nColor := CLR_VIOLET // RGB(255,153,255)  // Office_2003 PURPLE
      ELSEIF nClr3 == 6
         nColor := CLR_MEDIUMSLATEBLUE // RGB(118,170,235)  // TWITER
      ELSEIF nClr3 == 7
         nColor := CLR_MISTYROSE // RGB(225,237,204)  // 
      ELSEIF nClr3 == 8
         nColor := CLR_GAINSBORO // RGB(229,220,229)  //  
      ELSEIF nClr3 == 9
         nColor := CLR_LIGHTSKYBLUE // RGB(51,255,255)   //  
      ELSE
         nColor := CLR_WHITE
      ENDIF
   ENDIF
   ...
   IF nSumma == 1500
      nColor :=  CLR_SALMON // RGB(255,128,64) // ORANGE
   ENDIF
...
STATIC FUNCTION TsbColorFore( nAt, nCol, oBrw )
   ...
   IF nTsbColor <= -100    
      nColor :=  CLR_DARKSLATEGRAY  // HGRAY
   ELSEIF nTsbColor < 0         
      nColor := CLR_RED
...
FUNCTION TableExport(oBrw)   
   ...
   DEFINE CONTEXT MENU OF &cForm
       ...
       SEPARATOR                             
       MENUITEM  "Export to Excel (xml-files) 2"   ACTION ToExcel4(oBrw)  FONT Font3 
       SEPARATOR                             
       MENUITEM  "Exit"                            ACTION Nil             FONT Font7 
   END MENU
...
STATIC FUNCTION ToExcel1(oBrw)  
   ...
   aTitle     := { cTitle, hFontTitle }     // титул со своим фонтом - НЕ СДЕЛАНО !
...
STATIC FUNCTION ToExcel2(oBrw)  
   ...
   hFont := GetFontHandle( "Font_6" )         // указать свой фонт
   aFont := GetFontParam( hFont )
   nFSize := 14                               // указать свой размер фонта для Excel  
   cFontName := "Font_" + hb_ntos( _GetId() )  
   _DefineFont( cFontName, aFont[1], nFSize, aFont[3], aFont[4] )  
   ...
   hProgress  := test.PBar_1.Handle         // хенд для ProgressBar
   lSave      := .T.                        // сохранить файл
   bExtern    := nil  // подключение внешнего блока для оформления oSheet исп. объект Tsbrowse
                      // Например можно сделать так: bExtern := {|oSheet,oBrw| MyModify(oSheet, oBrw) }
