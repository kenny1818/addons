...
{
   PARAFORMAT2 parForm;
   int         left, center, right, nNumb, justify;
   ...
   justify = ( parForm.wAlignment == PFA_JUSTIFY  ) ? TRUE : FALSE;
   ...
   HB_STORL( justify, -1, 9 );
}

HB_FUNC( SETPARFORM )                              //SetParForm(HWND hwnd, BOOL left, BOOL Center, BOOL Right, int Tab, int Number, int StartIndent, int RightIndent, int Offset, BOOL Justify)
{
   LRESULT     lResult;
   PARAFORMAT2 parForm;
   DWORD       Mask = 0;
...
HB_FUNC( GETSELTEXT )                              //GetSelText(H)
{
   if( IsWinxpSp1Min() )
   {
      GETTEXTEX gtxt;
...
HB_FUNC( SETPARTABS )                              //SetParTabs(HWND hwnd , aTabs)
{
   ...
   parForm.cbSize = sizeof( PARAFORMAT2 );
   parForm.dwMask = PFM_TABSTOPS;
   parForm.cTabCount = (SHORT) (nArr + 1);
