...
PROCEDURE OpenFile( lProcess, nProcess )
   ...
   ELSE
      ...
      IF ! Empty( cFile )
         ...
         // win_RunDetached([CommandName], CommandLine, @ProcessId [, lNoConsole])
         win_RunDetached( NIL, AddQuote( cFile ), @nProcess, .F. )

         HMG_SysWait( .2 )

         IF Empty( nProcess )
            MsgStop( 'Can not found process ID.', 'Error' )
         ENDIF
...
FUNCTION GetExt( cFileName )  // 01/10/19 Pierpaolo Martinello

RETURN Lower( hb_FNameExt( cFilename ) )
