...
FUNCTION Brw2Dbf( oBrw, cFile, lActivate, hProgress, cVia, cCodePage ) 
   ...
   // удал€ем колонку, если есть Selector
   If oBrw:lSelector
      hb_ADel(aStruct, 1, .T.)
   EndIf
...
* =======================================================================================
FUNCTION Brw2Csv(oBrw, cFile, lActivate, hProgress )      
   LOCAL lDbfActivate, cAlias, cRdd, cCodePage, bOldError, lOpen, cDbfFile, cMsg, lCreate

   DEFAULT cFile := GetStartUpFolder() + "\Test.csv", lActivate := .T., hProgress := nil

   lCreate      := .F.
   lDbfActivate := .F.
   cRdd         := "DBFCDX"
   cCodePage    := "RU1251"
   cDbfFile     := ChangeFileExt( cFile, ".dbf" )
   Brw2Dbf(oBrw, cDbfFile, lDbfActivate, hProgress, cRdd, cCodePage )      

   bOldError := ERRORBLOCK( {|var| BREAK(var)} )  // ѕроста€ обработка ошибок открыти€ файлов
   BEGIN SEQUENCE
     cAlias   := "TMP"
     USE ( cDbfFile ) ALIAS ( cAlias ) VIA cRdd CODEPAGE cCodePage SHARED NEW
     lOpen := .T.
   RECOVER
     cMsg := "Database - ;" + cDbfFile + ";" 
     cMsg += "busy with another process !;;"
     cMsg := AtRepl( ";", cMsg, CRLF )
     MsgStop( cMsg , "Error" )
     lOpen := .F.
   ...
   IF lOpen
      ...
      //COPY TO (cFile) DELIMITED WITH TAB

      // Uses "|" as field delimiter
      //COPY TO (cFile) DELIMITED WITH PIPE

      // Encloses character values in single quotes and separates
      // fields with Chr(255)
      //COPY TO (cFile) DELIMITED WITH ( {"'", Chr(255) } )

      TMP->( DbCloseArea() )
      lCreate := .T.
      DELETEFILE(cDbfFile)
      cDbfFile  := ChangeFileExt( cDbfFile, IIF( cRdd == "DBFCDX", ".fpt", ".dbt" ) )
      DELETEFILE(cDbfFile)

   ENDIF

   If lActivate .AND. lOpen
      ShellExecute( 0, "Open", cFile,,, 3 )
   EndIf
