...
Function TBrw_Create( ControlName, ParentForm, nRow, nCol, nWidth, nHeight, uAlias, lCell, FontName, FontSize )
    ...
    If nWidth  < 0                                         // øèðèíà óìåíüøåíèÿ
       nWdt   := nWidth
       nWidth := 0
    EndIf
    
    If nWidth == 0                                         // ðàñ÷èòàòü øèðèíó
       nWidth := aRect[3]
    EndIf
    
    If nHeight < 0                                         // âûñîòà óìåíüøåíèÿ
       nHgt    := nHeight
       nHeight := 0
    EndIf
    
    If nHeight == 0                                        // ðàñ÷èòàòü âûñîòó
       nHeight := aRect[4] - nRow
    EndIf
    
    If nHgt < 0; nHeight += nHgt                           // óìåíüøèòü âûñîòó
    EndIf
    
    If nWdt < 0; nWidth  += nWdt                           // óìåíüøèòü øèðèíó
    EndIf
