...
PROCEDURE Main
   ...
   DEFINE WINDOW Form_0 ;
      ...
      WIDTH 600 ;
      HEIGHT 600 ;
      TITLE "(2) TsBrowse DBASE SHARED Demo" ;
      MAIN ;
      NOMAXIMIZE NOSIZE ;
   ...
   DEFINE STATUSBAR
      ...
      STATUSITEM "(2) TsBrowse - network opening of the database!" WIDTH 290 FONTCOLOR BLUE
      STATUSITEM " 00:00:00"
      KEYBOARD
   END STATUSBAR
...
// Новая запись в базе НЕ добавляется, а берется первая удалённая запись, 
// восстанавливаем эту удалённую запись
// A new entry in the database is NOT added, but the first deleted record is taken,
// restore this deleted record 
STATIC FUNCTION RecnoInsert(oBrw)
   LOCAL lAppend, nRecno

   If empty(oBrw:bAddBefore)
      oBrw:bAddBefore := {|ob|
                          Local lAdd := .T., a := ob:cAlias, nRec := 0
                          Local nOld := (a)->( IndexOrd() )
                          (a)->( OrdSetFocus('DEL') )
                          SET DELETED OFF
                          (a)->( dbGotop() )
                          If (a)->( !EOF() ) .and. (a)->( RLock() )
                             (a)->( dbRecAll() )
                             (a)->( DbUnlock() )
                             nRec := (a)->( RecNo() )
                             lAdd := .F.
   ...
   IF MsgYesNo( "To insert record in the table ?", "Сonfirmation" )

      // встроенный метод для добавления записи
      lAppend := oBrw:AppendRow(.T.)
...
STATIC FUNCTION RecnoDelete(oBrw)
   ...
   oBrw:aMsg[37] := "Delete an entry in the table ?"
   oBrw:aMsg[38] := "Delete row in table ?"
   oBrw:aMsg[39] := "Сonfirmation"
...
FUNCTION UseOpenBase()
   LOCAL aStr   := {} 
   LOCAL cDbf   := GetStartUpFolder() + "\TEST2" 
   LOCAL cIndx  := cDbf 
   LOCAL lDbfNo 
   LOCAL aAlias := {} 
   LOCAL n      := 0 
