/*
 * $Id: tmysql.prg 18755 2013-01-10 15:23:52Z vszakats $
 */
...
METHOD New( aRow, aFStruct, cTableName, cCollation) CLASS TMySQLRow
   ...
   IF VALTYPE(cCollation) == "C" .AND. ! EMPTY(cCollation)  // better check for real possible needed
     ::cMyCollation := cCollation
   ELSE
   ...
   IF nNum > 0 .AND. nNum <= Len( ::aRow )
      ...
      IF ::FieldType( nNum ) == "C"
         IF ! ::cMyCollation == SET(_SET_CODEPAGE)
           RETURN HB_TRANSLATE( PadR( ::aRow[ nNum ], ::aFieldStruct[ nNum ][ MYSQL_FS_LENGTH ] ), ::cMyCollation, SET(_SET_CODEPAGE) )
         ELSE
           RETURN PadR( ::aRow[ nNum ], ::aFieldStruct[ nNum ][ MYSQL_FS_LENGTH ] )
         ENDIF
      ELSEIF ::FieldType( nNum ) == "M"
         IF ! ::cMyCollation == SET(_SET_CODEPAGE)
           RETURN HB_TRANSLATE( ::aRow[ nNum ], ::cMyCollation, SET(_SET_CODEPAGE) )
         ELSE
           RETURN ::aRow[ nNum ]
         ENDIF
      ELSEIF ::FieldType( nNum ) == "L"      // elch
         RETURN IF( ASC(LEFT( ::aRow[ nNum ],1)) == 0, .F., .T.)
      ELSE
         RETURN ::aRow[ nNum ]
      ENDIF
   ENDIF
...
METHOD FieldType( nNum ) CLASS TMySQLRow
   ...
   IF nNum >= 1 .AND. nNum <= Len( ::aFieldStruct )
      ...
      CASE MYSQL_TYPE_MEDIUM_BLOB  // 2^24 not to big for 64k ?
         RETURN "M"

      CASE MYSQL_TYPE_BIT  //elch
         RETURN "L"
...
CREATE CLASS TMySQLQuery
   ...
   METHOD affected_rows()               // Call mysql_affected_rows() function
...
METHOD GetRow( nRow ) CLASS TMySQLQuery
   ...
   IF ::nResultHandle != NIL
      ...
      IF ::aRow != NIL
         ...
         oRow := TMySQLRow():New( ::aRow, ::aFieldStruct, NIL, ::cMyCollation )
...
METHOD affected_rows() CLASS TMySQLQuery // Call mysql_affected_rows() function
...
METHOD FieldGet( cnField ) CLASS TMySQLQuery
   ...
   IF nNum > 0 .AND. nNum <= ::nNumfields
      ...
      IF ::FieldType( nNum ) == "C"
         IF ! ::cMyCollation == SET(_SET_CODEPAGE)
            RETURN HB_TRANSLATE( PadR( Value,::aFieldStruct[ nNum ][ MYSQL_FS_LENGTH ] ), ::cMyCollation, SET(_SET_CODEPAGE) )
         ELSE
            RETURN PadR( Value,::aFieldStruct[ nNum ][ MYSQL_FS_LENGTH ] )
         ENDIF
      ELSEIF ::FieldType( nNum ) == "M"
         IF ! ::cMyCollation == SET(_SET_CODEPAGE)
            RETURN HB_TRANSLATE( Value, ::cMyCollation, SET(_SET_CODEPAGE) )
         ELSE
            RETURN Value
         ENDIF
      ELSE
...
METHOD New( nSocket, cQuery, cTableName, cCollation ) CLASS TMySQLTable
   ...
   ::super:New( nSocket, AllTrim( cQuery ), cCollation )
...
METHOD GetBlankRow( lSetValues ) CLASS TMySQLTable
   ...
   RETURN TMySQLRow():New( aRow, ::aFieldStruct, ::cTable, ::cMyCollation )
...
METHOD CreateTable( cTable, aStruct, cPrimaryKey, cUniqueKey, cAuto ) CLASS TMySQLServer
   ...
   FOR i := 1 TO Len( aStruct )
      ...
      CASE "C"
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " char(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")" + Eval( cNN, aStruct[ i ] ) + iif( aStruct[ i ][ DBS_NAME ] == cPrimaryKey, " NOT NULL ", "" ) + ","
         EXIT

      CASE "M"
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " text" + Eval( cNN, aStruct[ i ] ) + ","
         EXIT
      ...
      CASE "N"
         ...
         IF aStruct[ i ][ DBS_DEC ] == 0 .AND. aStruct[ i ][ DBS_LEN ] <= 18
            ...
            CASE aStruct[ i ][ DBS_LEN ] <= 2
               ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " tinyint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            CASE aStruct[ i ][ DBS_LEN ] <= 4
               ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " smallint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            CASE aStruct[ i ][ DBS_LEN ] <= 6
               ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " mediumint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            CASE aStruct[ i ][ DBS_LEN ] <= 9
               ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " int(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            OTHERWISE
               ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " bigint(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")"
            ENDCASE
      ...
      CASE "D"
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " date " + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      CASE "B"
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " mediumblob "  + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      CASE "I"
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " mediumint " + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      CASE "L"  //elch
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " bit "  + Eval( cNN, aStruct[ i ] ) + ","
         EXIT

      OTHERWISE
         ::cCreateQuery += qText( aStruct[ i ][ DBS_NAME ] ) + " char(" + hb_ntos( aStruct[ i ][ DBS_LEN ] ) + ")" + Eval( cNN, aStruct[ i ] ) + ","
...
METHOD Query( cQuery ) CLASS TMySQLServer
   ...
   IF nNumTables == 1
      oQuery := TMySQLTable():New( ::nSocket, cQuery, cTableName, ::cMyCollation )
   ELSE
      oQuery := TMySQLQuery():New( ::nSocket, cQuery, NIL, ::cMyCollation )
   ENDIF
...
// TOFIX: Conversion creates a .dbf with "C" fields of wrong dimension, maybe only when utf8 collation
METHOD TableStruct( cTable ) CLASS TMySQLServer

   LOCAL aStruct := {}
   LOCAL nRes, aField, aSField, i
   ...
   IF ! Empty( nRes )
      FOR i := 1 TO mysql_num_fields( nRes )
         ...
         * elminated this former validate
         * ASAIK indexes in MySQL ( Monty's Maria :) wont be fetched as regulary fields ?
         ...
         CASE MYSQL_TYPE_STRING  // CHAR_TYPE
            * cause of utf8 server ?? - exact 3 times longer as it should be
            * i correct this at higher prg level
            * TEST
            aSField[ DBS_TYPE ] := "C"
            IF ::cMyCollation == "UTF8"
              aSField[ DBS_LEN ] := MAX(1,INT( aField[ MYSQL_FS_LENGTH ] / 3))  // to be proofed if valid
            ENDIF
            EXIT

         CASE MYSQL_TYPE_TINY_BLOB  // 2^8 CHAR
         CASE MYSQL_TYPE_VARCHAR    // max 2^16 CHAR
         CASE MYSQL_TYPE_VAR_STRING
            IF aField[ MYSQL_FS_LENGTH ] <= 255
              aSField[ DBS_TYPE ] := "C"
            ELSE
              aSField[ DBS_TYPE ] := "M"  // no decimals as high byte
              aSField[ DBS_LEN ] := 10
            ENDIF
            EXIT
         ...
         CASE MYSQL_TYPE_BIT     // BIT - only first bit as boolean
            aSField[ DBS_TYPE ] := "L"
            aSField[ DBS_LEN ] := 1
            EXIT
         ...
         CASE MYSQL_TYPE_NEWDECIMAL
            aSField[ DBS_TYPE ] := "N"
            aSField[ DBS_LEN ] := MIN( aField[ MYSQL_FS_LENGTH ], 16)
            aSFIeld[ DBS_DEC ] := MIN( aField[ MYSQL_FS_DECIMALS ], 8)
            EXIT
